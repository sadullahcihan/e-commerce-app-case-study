...
const Filters = () => {
  ...
  const { brands, models, searchTerm, sortBy } = useSelector(state => state.products); // Redux state'ten alınan veriler
  ...
  const handleSortChange = (e) => {
    ...
    dispatch(setFilters({ searchTerm, sortBy: sortValue, brands: selectedBrands, models: selectedModels }));
  };

  // Handle Brand Selection Change
  const handleBrandChange = (value) => {
    setSelectedBrands(value);  // Update selected brands
    dispatch(setFilters({ searchTerm, sortBy, brands: value, models: selectedModels })); // Trigger filter update
  };

  // Handle Model Selection Change
  const handleModelChange = (value) => {
    setSelectedModels(value);  // Update selected models
    dispatch(setFilters({ searchTerm, sortBy, brands: selectedBrands, models: value })); // Trigger filter update
  };
